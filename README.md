...
- **Core/DWH** — приводит данные к правильным типам и складывает единую версию
  правды по сущностям (поездки, зоны).
- **Data Mart** — денормализованная витрина, оптимизированная под скорость чтения
  для BI, а не под целостность.

Это классический ELT-подход (raw → core → dm / bronze → silver → gold), который
используется в реальных компаниях для аналитических контуров.
...
down` (без флага `-v`!) + `docker compose up -d`, а не `stop`/`start` — подробнее
в комментарии над сервисом `greenplum` в `docker-compose.yml`.
...
## Известные ограничения и что было бы сделано иначе для прод-масштаба

- Сейчас пайплайн работает как full-reload (пересоздание таблиц на каждый запуск) —
  для одного месяца данных это нормально; для многолетней истории потребовалась бы
  инкрементальная загрузка по партициям.
- Период загрузки параметризован через Airflow params, но в проде разумнее вынести
  в Airflow Variables/расписание.
- Нет алертинга при падении DAG (email/Slack notification) — следующий шаг для
  продакшн-готовности.
